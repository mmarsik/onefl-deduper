...
class LinkGenerator():
    ...
    def _process_patient_row(cls, patid, pat_hashes, hash_uuid_lut,
        ...
        session = db.get_db_session(db.get_db_engine(config), create_tables=True)
    ...
    @classmethod
    def _process_patient_row_helper(cls, args):
        """
        """
        return cls._process_patient_row(*args)
    
    @classmethod
    def _process_patient_row_main(cls, pool, df, df_source, rules, hash_uuid_lut, 
                                  rules_cache, config, partner_code):
        """
        """
        l_args = [(row['PATID'], 
                   {rule: row[rule] for rule in rules if row[rule] != ''}.copy(), 
                   hash_uuid_lut, rules_cache, config, partner_code) for index, row in df_source.iterrows()]
        l_results = pool.map(cls._process_patient_row_helper, l_args)
        
        #    TODO: setup loop over results to extract links and to_investigate
        #    Kludgy as we iterate again over df_source.iterrows() to extract PATID
        investigations = []
        for frame, row in zip(l_results, df_source.iterrows()):
            patid = row[1]['PATID']
            links = frame[0]
    ...
    def _process_frame(cls, config, session, df_source, partner_code):
        ...
        patients_with_no_hashes = [row['PATID'] for index, row in df_source.iterrows() if len({rule: row[rule] for rule in rules if row[rule] != ''}) < 1]

        #    TODO: Insert multiprocessing pool here
        pool = mp.Pool(processes=NUM_CPUS)

        df, investigations = cls._process_patient_row_main(cls, pool, df, df_source, rules, hash_uuid_lut, 
                                          rules_cache, config, partner_code)
